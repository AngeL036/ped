...
def _construir_html(Venta_id:int, total: float, detalles: list[dict]) -> str:
    ...
    for d in detalles:
        ...
        <tr>
            ...
            <td style="padding:8px 12px; border-bottom:1px solid #f0f0f0;">{d['cantidad']}</td>
            <td> style="padding:8px 12px; border-bottom:1px solid #f0f0f0;">${d['precio_unitario']:.2f}</td>
            <td> style="padding:8px 12px; border-bottom:1px solid #f0f0f0;">${d['subtotal']:.2f}</td>
        </tr>"""
...
def enviar_ticket_correo(
    ...
    Lanza excepcion si falla - el router la convierte en HTTP 502.
    """
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Ticket de compra - Folio #{venta_id}"
